.PHONY: build dev test run dev-run stress std compiledb compiledb-commands check format clean help
...
stress:
	zig build -Doptimize=Debug stress -- $(ARGS)
...
help:
	...
	@echo "  make stress     - run the allocator stress-test stub (ARGS='--allocator system')"
